...
public class DemoActorImpl extends AbstractActor implements DemoActor, Remindable<Integer> {
    ...
    public Mono<Integer> incrementAndGet(int delta) {
        getActorStateManager().contains("accountId")
                .flatMap(exists -> exists ? super.getActorStateManager().get("accountId", int.class) : Mono.just(0))
                .map(c -> c + delta).flatMap(c -> super.getActorStateManager().set("accountId", 1).thenReturn(1));


        getActorStateManager().contains("accountholdername")
                .flatMap(exists -> exists ? super.getActorStateManager().get("accountholdername", int.class) : Mono.just(0))
                .map(c -> c + delta).flatMap(c -> super.getActorStateManager().set("accountholdername", 2).thenReturn(2));


                getActorStateManager().contains("accountbalance")
                .flatMap(exists -> exists ? super.getActorStateManager().get("accountbalance", int.class) : Mono.just(0))
                .map(c -> c + delta).flatMap(c -> super.getActorStateManager().set("accountbalance", 20000).thenReturn(20000));


                getActorStateManager().contains("acctbalance")
                .flatMap(exists -> exists ? super.getActorStateManager().get("acctbalance", int.class) : Mono.just(0))
                .map(c -> c + delta).flatMap(c -> super.getActorStateManager().set("acctbalance", 20001).thenReturn(20001));

        // super.getActorStateManager().set("accountId", 1);

        // super.getActorStateManager().set("accountholdername", 2);
        
        // super.getActorStateManager().set("accountbalance", 20000);
        
        // super.getActorStateManager().set("acctbalance", 20000);
